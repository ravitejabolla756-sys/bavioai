if (process.env.USE_INMEMORY_SESSION_STORE === 'true' || !process.env.REDIS_URL) {
    module.exports = require('./inMemorySessionStore');
} else {
    const { randomUUID } = require('crypto');
    const { getRedisClient } = require('../redisClient');

    const SESSION_TTL_SECONDS = parseInt(process.env.REALTIME_SESSION_TTL_SECONDS || '3600', 10);
    const SESSION_DEBUG = process.env.REDIS_SESSION_DEBUG === 'true';

    function logRedisOp(action, details) {
        if (SESSION_DEBUG) {
            console.log(`[sessionStore:${action}]`, JSON.stringify(details));
        }
    }
    ...
    class RealtimeSessionStore {
        async create({ businessId, assistantId, systemPrompt, welcomeMessage, voice, metadata }) {
            ...
            logRedisOp('create', { sessionId: session.id, businessId });
        ...
        async update(sessionId, patch) {
            logRedisOp('update', { sessionId, patchKeys: Object.keys(patch) });
        ...
        async appendTranscript(sessionId, entry) {
            ...
            logRedisOp('appendTranscript', { sessionId, role: transcriptEntry.role });
        ...
        async addConnectionMetadata(sessionId, connectionId, metadata) {
            ...
            logRedisOp('addConnection', { sessionId, connectionId, kind: metadata.kind, provider: metadata.provider });
        }
        ...
        async removeConnectionMetadata(sessionId, connectionId) {
            ...
            await withExpiry(client, connectionKey(sessionId));
            logRedisOp('removeConnection', { sessionId, connectionId });
        }
        ...
        async createCallMapping(provider, callSid, sessionId, metadata = {}) {
            ...
            logRedisOp('createCallMapping', { provider, callSid, sessionId });
        }

        async getSessionIdByCall(provider, callSid) {
            const client = await getRedisClient();
            const sessionId = await client.get(callMapKey(provider, callSid));
            logRedisOp('getSessionIdByCall', { provider, callSid, sessionId });
            return sessionId;
        }
        ...
        async storeCallMetadata(provider, callSid, metadata) {
            const client = await getRedisClient();
            await client.set(callMetadataKey(provider, callSid), JSON.stringify(metadata), { EX: SESSION_TTL_SECONDS });
            logRedisOp('storeCallMetadata', { provider, callSid });
        }

        async getCallMetadata(provider, callSid) {
            const client = await getRedisClient();
            const payload = await client.get(callMetadataKey(provider, callSid));
            logRedisOp('getCallMetadata', { provider, callSid, found: Boolean(payload) });
        ...
        async deleteSession(sessionId) {
            ...
            logRedisOp('deleteSession', { sessionId });
        }
    }
